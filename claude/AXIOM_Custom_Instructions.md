...
Growing the graph has two complementary aims: (1) a mechanistic knowledge graph of aging, holding causal chains, closures, and cross-paper synthesis; and (2) a dossier of genes and proteins with a documented role in aging and age-related disease, each carrying its manipulation-to-phenotype evidence. The second aim is a deliberate breadth goal that feeds downstream work (the COMPASS project). It means broad, well-grounded per-gene coverage is wanted, not only the highest-order synthetic claims.
...
1. Corpus first, graph second. Before proposing an entry, ask whether standard corpus retrieval would already give it in usable form. If yes, the graph adds nothing by restating it. Graph entries earn their place by holding what corpus search alone cannot, typically structured causal claims and cross-paper synthesis. A structured causal claim is exactly what corpus search does not return in usable form, so a single-gene manipulation-to-phenotype fact does not fail this guideline merely because the sentence stating it is retrievable: the edge is the structuring, and that is the value add. "Fails guideline 1" (guideline 6) is for claims already held as graph structure, or so generic that the corpus prose is itself the usable form (broad textbook statements, restatements of an existing edge), not for un-promoted causal facts that happen to sit in a retrievable chunk.

2. Filter for engineering-actionability. The project goal is "what molecular engineering would extend lifespan." Good entries should plausibly help answer that. Acceptable: a knob to turn, a failure mode to fix, a chain from intervention to outcome, a closure that explains why aging compounds, and a gene's or protein's documented role in aging or an age-related disease (a manipulation-to-phenotype fact: knockout, overexpression, or pharmacological modulation that changes lifespan, healthspan, or an age-related-disease outcome). This last category is the gene dossier (see Purpose); it is acceptable even as a single animal-model result, because the identity of a causal aging gene is itself durable, engineering-relevant content. Default rejects: methodological details; standalone quantitative measurements carrying no causal claim (a fold-change, a q-value, an EC50, an age coefficient); and phenotypic descriptors that name no driver or intervention point. The distinction that matters: "gene X knockout extends lifespan in mice" is a genetic-contribution fact and is acceptable; "protein X plasma level rises with age, coefficient 5.3e-3" is a standalone measurement and is not, unless it grounds a causal claim.
...
5. Volume is a trigger to re-examine, not a cap. A high entry count is a prompt to stop and check one thing: are the entries distinct, grounded, engineering-relevant claims, or are they paper-content reproduction (a methods section transcribed sentence-by-sentence, every quantitative result restated, one finding split across many rows)? If the former, the count is fine and the commit proceeds however large it is: a review or catalog paper naming twenty distinct genetic contributions to aging legitimately yields twenty-plus entries. If the latter, guidelines 1 and 2 are being under-applied, and the fix is to cut the reproduction, not to cap the total. This guideline never licenses dropping a groundable, engineering-relevant claim to hit a number; volume control comes from guidelines 1 and 2 at the candidate-claim level (see guideline 6). Most single primary-research papers still land in a modest range, so a large count on such a paper is a genuine signal to re-examine; a catalog or review paper enumerating many genes is the expected exception. The one hard constraint is review pacing: if a commit is too large to present for review, split it into reviewable batches rather than producing into a void.

6. Skip with reason, not with silence. When you identify a candidate claim during an SOP and choose not to promote it, log it in the "Considered but skipped" section of the response with a one-line reason. Valid reasons reduce to: fails guideline 1 (already implicit in corpus retrieval), fails guideline 2 (no engineering hook), fails QG1 (insufficient grounding), handoff to SOP_entity_reconciliation, handoff to SOP_edge_audit. Invalid reasons (do not use): "enough proposed already", "edit budget reached", "frequency threshold", "coverage threshold not met yet", "will come up in a future paper", "the source reports a negative result" (see guideline 7), "low confidence" without grounding rationale. These hide claims that deserve user judgment under the guise of editorial restraint. Volume control comes from guidelines 1 and 2 acting at the candidate-claim level, not from arbitrary caps on the final proposal. If you are tempted to skip for an invalid reason, promote the claim instead and let the user filter at review.

7. Negative results are content, not omissions. When a source tests a relation and does not find it, that is a finding about the biology and it belongs in the graph, recorded as an evidence row carrying `assertion_status: "refuting"`. A refuting row must supply `method` or `justification`, so a later reader can see what was tested rather than only that something failed. The edge-level rollup is derived and never stored: an edge whose evidence is entirely refuting derives as `refuted` and is excluded from all five analysis passes and from the build report's grounding metrics, and an edge carrying both kinds derives as `contested`, which stays in the analyses but caps any candidate touching it at `watch_item`. Two consequences for extraction. First, "the paper found no effect" is never a reason to skip a candidate claim; propose the edge and mark the row refuting. Second, when the corpus refutes something the graph already asserts, add the refuting row to the existing edge rather than deleting it, so the graph records that the question was asked and answered and the same claim is not re-proposed from the same paper months later. Deletion is for claims that should never have been made; `SOP_edge_audit.md` holds the discriminator.
...
- **Dataset ingestion** (`SOP_dataset_ingestion.md`). A structured dataset (a table, CSV/TSV, or database export whose grain is one row per record rather than one paragraph per chunk) needs mapping into graph nodes, edges, and conditions. This is the non-corpus sibling of paper extraction: use it when the source is rows, not prose. Trigger: "Ingest the [dataset] into the graph", "Run dataset ingestion on [file or export]", "Process the [dataset/table] into the graph", "Load this [table/genome table/interventions table] as graph nodes", or any request naming the ITP DCC export or a comparable structured table as the thing to be graphed. Note that "process this dataset" routes here, while "process this paper" routes to paper extraction; the discriminator is whether the source is a table of records or a prose document. This SOP has a hard provenance gate: dataset rows have no valid `grounding_type` under the current schema, so the procedure runs its mapping and entity-resolution steps but stops before any write until that schema decision is resolved in AXIOM Developer. Do not repurpose an existing `grounding_type` to force a write.
...
8. Assertion polarity is explicit on every evidence row and observation. `assertion_status` is `asserting` by default and `refuting` when the source tested the claim and did not find it; a refuting row must carry `method` or `justification`. Never encode a negative result by inverting the edge type, by burying it in `notes`, or by scoping it away in a condition. All three are invisible to the analysis layer, which reads `assertion_status` and nothing else, and inverting the edge type actively asserts a claim the source did not make. When two evidence rows on one edge disagree because they tested different things, scope the distinguishing condition to a single row via `evidence_id` rather than applying it to the whole edge.
...
- Refutations: the edge (existing edge_id or a new edge), the claim being refuted stated in the direction the source tested it, the refuting evidence row with its method and chunk citation, and the resulting derived state (refuted, or contested when the edge already carries asserting evidence).
...
Some SOPs add specialized sections to this base format (edge audit adds "Manual-handling items"; entity reconciliation adds "Manual-handling items"; dataset ingestion adds "Manual-handling items" and, when the provenance gate is unresolved, stops at the mapping stage without a commit). Follow the SOP's format when one is invoked.
